...
import type { PreventableEvent } from '../createPreventableEvent'
import type {
  ESCAPE_KEY,
  FOCUS_OUTSIDE,
...
import { createPreventableEvent } from '../createPreventableEvent'
import { focus } from '../focus'
import { markOutsideElementsAsHidden } from '../markOutsideElementsAsHidden'
...
export function usePopup<Props extends UsePopupProps>({
  initialFocus,
  returnFocus,
  onEscapeKeyDown,
  ...
  const handleFocusCapture = useEvent(
    (event: React.FocusEvent<HTMLDivElement>) => {
      popupManagerRef.current?.markFocusTargetAsInside(entry, event.nativeEvent)
  ...
  const forceDismiss = useEvent((request: PopupDismissRequest<'ancestor'>) => {
    ...
    return () => {
      queueMicrotask(() => {
        ...
        try {
          ...
          if (
            topmostModalEntry &&
            !topmostModalEntry.elementRef.current?.contains(returnFocusElement)
          ) {
            returnFocusElement =
